...
class SwisstopoTileDownloader {
  static const String stacBaseUrl =
      "https://data.geo.admin.ch/api/stac/v0.9/collections/ch.swisstopo.swisssurface3d-raster/items"; //to this url we will add the bounding box made from the radius set by the user
  Future<List<TileMetadata>> fetchTileUrls(
    ...
    final bbox =
        "$minLon,$minLat,$maxLon,$maxLat"; //create string from bounding box coordinates to pass as parameter to stac api
  ...
  */
  {
    Directory(outputDirectory).deleteSync(
      recursive: true,
    ); //should cache existing tiles if they're reused here
    final files = <File>[];
